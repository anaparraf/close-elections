...
munic_deams_2018 <- read_excel("dados/munic_deams.xlsx") |> 
...
mun_close <- mun |>
  ...
  left_join(elections_final, by = c('code_muni' = 'codigo_ibge'))

margem_vit_geral <- mun_close |> 
  ggplot(aes(fill=margem_vitoria_mulher))+
  geom_sf()+
  geom_sf(color = NA)+
  scale_fill_viridis(option = 'A', na.value = 'lightgray')+
    labs(title = 'Margem de vitória das mulheres no Brasil',
       subtitle = 'Eleições entre um homens e mulheres (2004-2012)',
       color = NULL,
       fill = 'Margem de vitória') +
  theme_void()+
     theme(plot.title = element_text(hjust = 0.5),
             plot.subtitle = element_text(hjust = 0.5))

ggsave('margem_vit_geral.png', plot = margem_vit_geral, dpi=600 )
...
pessoaeleita <- mun_close |> 
  ...
  # geom_sf(color = "black") +
  geom_sf(color = NA)+
  scale_fill_manual(values = c('Outro' = 'whitesmoke', 'Mulher Eleita' = '#CD3333', 'Homem Eleito' = '#53868B'))+
      # labs(title = 'Close Elections no Brasil',
      #  subtitle = ' ',
      #  color = NULL) +
...
# ggsave("img/pessoaeleita.png", plot = pessoaeleita, dpi = 600)
...
```{r}
 ...
 gambiarra <- mun_close |> 
  mutate(tx_homicidio_domestico = if_else(is.na(tx_homicidio_domestico), 100000, tx_homicidio_domestico))
...
gambiarra |> 
  ggplot(aes(fill=tx_homicidio_domestico))+
  geom_sf()+
  geom_sf(color = 'lightgray')+
          # fill = 'white')+
  # geom_sf(data = gambiarra == 100000)
  geom_sf(data = gambiarra[gambiarra$tx_homicidio_domestico == 100000, ], color = 'white', fill = NA) +  # Destacando com borda vermelha
...
# ggsave("img/transparente.png", plot = transparente, dpi = 600)
...
```{r}
todas_candidatas <- read.csv('dados/candidatas_graf.csv')


candidatas <- todas_candidatas |> 
  group_by(codigo_ibge) |>
  summarise(mulher_eleita = any(mulher_eleita == TRUE)) 

todas_candidatas |> 
  filter(is.na(mulher_eleita))
...
mun_candidatas <- mun |>
  # mutate(code_muni = as.character(code_muni)) |> 
  left_join(candidatas, by = c('code_muni' = 'codigo_ibge'))
mun_candidatas
```
...
mun_candidatas |> 
  ...
  # geom_sf(color = "black") +
  geom_sf(color = NA)+
  scale_fill_manual(values = c('Outro' = 'whitesmoke', 'Mulher Eleita' = '#FF7373', 'Homem Eleito' = '#7373FF'))+
      # labs(title = 'Close Elections no Brasil',
      #  subtitle = ' ',
      #  color = NULL) +
...
```{r}
# grafico variação por ano mulher eleita
mulhereleita_2004 <- mun_close |> 
  filter(ano == 2004) |> 
  mutate(mulher_eleita = case_when(
    mulher_eleita == FALSE ~ 'Homem Eleito',
    mulher_eleita == TRUE ~ 'Mulher Eleita',
  ...
  geom_sf(color = NA)+
  scale_fill_manual(values = c('Outro' = 'whitesmoke', 'Mulher Eleita' = '#FF7373', 'Homem Eleito' = '#7373FF'))+
    labs(title = 'Eleições entre homens e mulheres no Brasil',
       subtitle = '2004',
       color = NULL,
       fill = 'Pessoa eleita') +
  theme_void()+
...
ggsave("img/2004.png", plot = mulhereleita_2004, dpi = 600) 


```

```{r}
mulhereleita_2008 <- mun_close |> 
  filter(ano == 2008) |> 
  mutate(mulher_eleita = case_when(
    mulher_eleita == FALSE ~ 'Homem Eleito',
    mulher_eleita == TRUE ~ 'Mulher Eleita',
...
ggsave("img/2008.png", plot = mulhereleita_2008, dpi = 600) 
```


```{r}
mulhereleita_2012 <- mun_close |> 
  filter(ano == 2012) |> 
  mutate(mulher_eleita = case_when(
    mulher_eleita == FALSE ~ 'Homem Eleito',
    mulher_eleita == TRUE ~ 'Mulher Eleita',
...
ggsave("img/2012.png", plot = mulhereleita_2012, dpi = 600) 
```
...
brasil <-  mun_close |> 
  ggplot()+
  geom_sf(aes(color = 'red', fill='white')) +
  geom_sf() 
